...
A command-line tool to bulk create, manage, and check New Relic Synthetics
monitors using the NerdGraph API.

Key metrics include:
- Jobs per Minute (by monitor type and result)
- Heavyweight Throughput (jobs/min)
- Lightweight Throughput (jobs/min)
...
# Configuration is sourced from environment variables for security and flexibility.
API_KEY = os.getenv("NEW_RELIC_API_KEY")
ACCOUNT_ID = os.getenv("NEW_RELIC_ACCOUNT_ID")
PRIVATE_LOCATION_GUID = os.getenv("NEW_RELIC_PRIVATE_LOCATION_GUID")

# The single endpoint for all New Relic NerdGraph API requests.
NERDGRAPH_URL = "https://api.newrelic.com/graphql"


class Colors:
    """A utility class to hold ANSI escape codes for coloring terminal output."""
    YELLOW = '\033[93m'
    CYAN = '\033[96m'
    GREEN = '\033[92m'
    RED = '\033[91m'
...
def validate_env_vars(for_creation=False):
    """
    Validates that necessary environment variables are set before proceeding.
    Exits the script with an error message if any are missing.

    Args:
        for_creation (bool): If True, also validates the private location GUID,
                             which is required for creating new monitors.
    """
    if not API_KEY or not ACCOUNT_ID:
        print(f"{Colors.RED}❌ Error: NEW_RELIC_API_KEY and NEW_RELIC_ACCOUNT_ID must be set.{Colors.ENDC}")
        sys.exit(1)
    if for_creation and not PRIVATE_LOCATION_GUID:
        print(f"{Colors.RED}❌ Error: NEW_RELIC_PRIVATE_LOCATION_GUID must be set for creation.{Colors.ENDC}")
        sys.exit(1)

# --- Synthetics Script Templates ---
# These are default, boilerplate scripts used when creating new scripted monitors.
...
def make_nerdgraph_request(query, variables=None):
    """
    Sends a request to the NerdGraph API and handles the response.
    ...
    Returns:
        tuple: A tuple containing (success_boolean, payload_dict).
               On failure, the payload contains an 'error' key.
    """
    ...
    try:
        ...
        # NerdGraph can return errors in a few different places in the response body,
        # so we check all common locations for a robust error handling.
...
def create_ping_monitor(name, url, location_guid, period):
    """Creates a new Synthetics SIMPLE (Ping) monitor."""
...
def create_simple_browser_monitor(name, url, location_guid, period):
    """Creates a new Synthetics BROWSER (Simple Browser) monitor."""
...
def create_scripted_browser_monitor(name, url, location_guid, period):
    """Creates a new Synthetics SCRIPT_BROWSER monitor using a template script."""
    print(f"  Creating SCRIPT_BROWSER monitor: '{name}'...", end="", flush=True)
    script_text = SYNTHETIC_BROWSER_SCRIPT_TPL % url
    # The script text must be escaped as a JSON string to be embedded in the GraphQL mutation.
...
def create_scripted_api_monitor(name, url, location_guid, period):
    """Creates a new Synthetics SCRIPT_API monitor using a template script."""
...
def find_monitors_by_tag():
    """
    Finds all Synthetics monitors with the 'ManagedBy: BulkScript' tag.

    Returns:
        list: A list of monitor entity dictionaries, or an empty list on failure.
    """
    print("🔍 Finding monitors with tag 'ManagedBy:BulkScript'...")
    # This NerdGraph query uses entitySearch to find all Synthetic monitors
    # with a specific tag, which is the key to managing them with this script.
...
def update_monitor_status(guid, name, monitor_type, monitor_id, status):
    """
    Enables or disables a specific Synthetics monitor.

    Args:
        guid (str): The monitor's entity GUID.
        name (str): The monitor's name (for display).
        monitor_type (str): The type of the monitor (e.g., 'SIMPLE', 'BROWSER').
        monitor_id (str): The monitor's legacy ID (for display).
        status (str): The desired status, either 'ENABLED' or 'DISABLED'.
    """
    action_verb = "Enabling" if status == "ENABLED" else "Disabling"
    print(f"  {action_verb} monitor '{name}' (ID: {monitor_id})...", end="", flush=True)

    # Each monitor type has a different NerdGraph mutation for updates.
    # This map allows us to dynamically select the correct one.
...
def delete_monitor(guid, name, monitor_id):
    """
    Deletes a specific Synthetics monitor.
...
def parse_selection(selection_str, max_index):
    """
    Parses a user input string into a set of selected indices.
    Accepts formats like '1,3,5', '2-4', 'ALL', or a mix.

    Args:
        selection_str (str): The user's input string.
        max_index (int): The maximum valid index.

    Returns:
        set: A set of integer indices corresponding to the user's selection.
    """
...
def handle_check_results(args):
    """
    Fetches and displays a summary of SyntheticCheck results using a NRQL query.
    Calculates jobs per minute and provides a throughput summary.
    """
    validate_env_vars()
    ...
    # Allow filtering results by monitor type if specified by the user.
    type_filter = ""
    ...
    # This NRQL query aggregates check results, faceting by result (SUCCESS/FAILURE)
    # and monitor type to provide a high-level summary of health.
    ...
    # Calculate throughput totals to distinguish between lightweight (ping)
    # and heavyweight (browser/API) checks, which have different costs.
...
if __name__ == "__main__":
    # Map monitor type names to their respective creation functions.
    ...
    # Map user-friendly choices to the required NerdGraph period enumeration.
    ...
    if choice == "1":
        ...
        for type_name in types_to_create:
            if type_name not in MONITOR_TYPE_MAP:
                print(f"⚠️ Skipping unknown type: {type_name}"); continue
            ...
            for _ in range(quantity):
                ...
                time.sleep(0.5) # A short delay to avoid hitting API rate limits.
        print("\n✨ Creation complete.")
    ...
    # --- Actions: Enable, Disable, or Delete Monitors ---
    else:
        validate_env_vars()
        all_monitors = find_monitors_by_tag()

        # Filter the list of all found monitors based on user-provided type arguments.
        ...
        # Further filter monitors to only show those relevant to the chosen action.
        ...
        # Display the list of manageable monitors to the user.
        ...
        # Prompt for user selection and confirmation.
